...
import ca.uwaterloo.flix.language.ast.{ReducedAst, SimpleType, SourceLocation, Symbol, Type, TypeConstructor}
...
object JvmOps {
  ...
  def getTagTypesOf(enums: Map[Symbol.EnumSym, MonoEnum])(implicit root: Root): Iterable[BackendObjType.TagType] = {
    val res = mutable.Set.empty[BackendObjType.TagType]
    for (enm <- enums; caze <- enm._2.cases.values) {
      caze.tpes match {
        case Nil => res.add(BackendObjType.NullaryTag(caze.sym.name))
        case nonEmpty => res.add(BackendObjType.Tag(nonEmpty.map(BackendType.toBackendType)))
      }
    }
    res.toSet
  }

  def getStructTypesOf(structs: Map[Symbol.StructSym, MonoStruct])(implicit root: Root): Iterable[BackendObjType.Struct] = {
    val res = mutable.Set.empty[BackendObjType.Struct]
    for (struct <- structs.values) {
